...
const DataProvider = ({ children }) => {
  ...
  const authClick = async (isLogin = false) => {
    ...
    const apiLink = isLogin
      ? "https://bulk-email-sender-backend.onrender.com/api/v2/login"
      : "https://bulk-email-sender-backend.onrender.com/api/v2/signup";
  ...
  const setupSMTP = async (smtpData) => {
    ...
    try {
      const data = await axios.post(
        "https://bulk-email-sender-backend.onrender.com/api/v2/setup-smt",
        smtpData,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  const createEmailList = async (data) => {
    ...
    try {
      ...
      await axios.post(
        "https://bulk-email-sender-backend.onrender.com/api/v2/createEmailList",
        data,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  const getEmailList = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://bulk-email-sender-backend.onrender.com/api/v2/getEmailList",
        data,
        {
          headers: {
            "Content-Type": "application/json",
